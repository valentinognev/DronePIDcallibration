...
**Status:** Functional prototype with all major analysis tools scaffolded; UI and algorithm parity with the original MATLAB app is incomplete in places (see [Known gaps](#known-gaps--next-work) below). Recent Log Viewer work: PX4 `.ulg` support, per-trace autoscale/titles, analysis-window slider (see [UPDATES.md](UPDATES.md)).
